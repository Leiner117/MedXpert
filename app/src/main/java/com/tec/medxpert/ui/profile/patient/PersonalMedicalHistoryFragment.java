...
/**
 * Fragment for managing patient's personal medical history
 */
@AndroidEntryPoint
public class PersonalMedicalHistoryFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        titleTextView.setText("Personal Medical History");
        ...
        saveButton.setOnClickListener(v -> {
            viewModel.updatePersonalMedicalHistory(conditionsList);
        });
        ...
        viewModel.getPatientData().observe(getViewLifecycleOwner(), resource -> {
            if (resource.status == Resource.Status.LOADING) {
                ...
                if (resource.status == Resource.Status.SUCCESS && resource.data != null) {
                    ...
                    if (patient.getPersonalData() != null && patient.getPersonalData().getPersonalMedicalHistory() != null) {
                        conditionsList = new ArrayList<>(patient.getPersonalData().getPersonalMedicalHistory());
                        updateConditionsList();
                    }
        ...
        viewModel.getSaveStatus().observe(getViewLifecycleOwner(), resource -> {
            if (resource.status == Resource.Status.LOADING) {
                ...
                if (resource.status == Resource.Status.SUCCESS && Boolean.TRUE.equals(resource.data)) {
                    Toast.makeText(getContext(), "Personal medical history saved successfully", Toast.LENGTH_SHORT).show();
                    requireActivity().onBackPressed();
                } else if (resource.status == Resource.Status.ERROR) {
                    Toast.makeText(getContext(), resource.message, Toast.LENGTH_SHORT).show();
                }
    ...
    private void updateConditionsList() {
        ...
        if (conditionsList.isEmpty()) {
            ...
            emptyText.setText("No medical conditions added");
    ...
    private void showEditDialog(int position) {
        ...
        builder.setTitle("Edit Medical Condition");
